		...
		http|https)
			while true; do
				...
				if [ "$duplicate_domain" -eq 1 ]; then
					echo "新增失敗：域名$custom_domains 已存在，请重新输入其他域名。"
					continue
				fi
				break
			done
...
delete_forwarding_service() {
	...
	read -e -p "确认删除服务 $service_name嗎？ (y/N):" confirm_delete
...
kpanel_node_join() {
	...
	if [ "$saved_fingerprint" = "$fingerprint" ] && kpanel_node_safe_regular_file "$KPANEL_NODE_CONFIG"; then
		...
		if [ "$staged_fingerprint" != "$fingerprint" ]; then
			kpanel_node_clear_enrollment_stage || {
				echo "本機存在無法安全復原的 KPanel 節點授權暫存；請檢查${KPANEL_NODE_ENROLLMENT_STAGE}。" >&2
				return 1
			}
